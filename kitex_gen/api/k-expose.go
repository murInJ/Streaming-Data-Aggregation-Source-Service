...
func (p *PullExposeStreamResponse) fastWriteField3(buf []byte, binaryWriter bthrift.BinaryWriter) int {
	offset := 0
	offset += bthrift.Binary.WriteFieldBegin(buf[offset:], "source_msg", thrift.STRUCT, 3)
	offset += p.SourceMsg.FastWriteNocopy(buf[offset:], binaryWriter)
	offset += bthrift.Binary.WriteFieldEnd(buf[offset:])
	return offset
}
...
func (p *PullExposeStreamResponse) field3Length() int {
	l := 0
	l += bthrift.Binary.FieldBeginLength("source_msg", thrift.STRUCT, 3)
	l += p.SourceMsg.BLength()
	l += bthrift.Binary.FieldEndLength()
	return l
}
